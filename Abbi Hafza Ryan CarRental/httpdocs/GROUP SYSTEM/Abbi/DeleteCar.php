<!--
•	Name of screen: DeleteCar.php 
•	Purpose of screen: Php file for deleting a car from the database
	If car is out on rental then they cannot delete the car 
	This is done by seeing if the status of the car is Unavailable or Available
•	|Student name:Abigail Murray|Student Number:C00260073|Date: March 2024|
-->

<?php session_start();
//starts a PHP session.Sessions store information (in session variables) to be used across multiple pages during a user's visit to a //website.
...
<?php 
	...
	//need to get carType from POST data
	$carTypeSelected = $_POST['delcarType'];

	//SQL Query--retrieves the carTypeID from carType table based on modelName of car selected
	$queryCarType = "SELECT carTypeID from carType WHERE modelName = '$carTypeSelected'"; 
	$resultQuery =  mysqli_query($con, $queryCarType);//executes the query, stores result in $resultQuery

	// Get the car ID to be deleted from the POST data
	$carID = $_POST['delid'];

	//SQL Query-- checks if car is out on rental by selecting the currentStatus from the car table 
	// ('unavailable' indicates it's out on rental)
	$queryStatus = "SELECT currentStatus FROM car WHERE carID = '$carID' AND currentStatus = 'Unavailable'"; 
	$resultStatus = mysqli_query($con, $queryStatus);//execute query and store result in $resultStatus
	...
	if(mysqli_num_rows($resultStatus) > 0)
		{
			...
			//Redirect to the page and dont let them delete car
